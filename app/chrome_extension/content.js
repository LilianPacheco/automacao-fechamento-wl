...
async function wlUploadAttachment(config, item, source, sourceIndex) {
  ...
  if (await wlSendAttachment(payload)) return true;
  await wlSleep(250);
  return wlSendAttachment(payload);
}
...
async function wlCollectPeriodEvidence(panel, config, base) {
  ...
  for (let step = 0; step < 300; step += 1) {
    ...
    if (jumpButtonFound && olderText && olderLoads < 20 && !wlHasDate(panel, config.start_label)) {
  ...
  return {
    ...wlSummarizeInventory(inventory),
    start_date_observed: inventory.some(
      (item) => item.message_date === config.start_label
    ),
...
async function wlCollectSequentialPeriodEvidence(panel, config, base) {
  ...
  const startFound = await wlNavigateToDate(panel, config);
  await wlSleep(400);
  await wlCloseMessageSearch();
  panel = document.querySelector('[data-testid="conversation-panel-messages"]') || panel;
  if (!startFound) {
...
async function wlAnalyze(config) {
  ...
  const evidence = await wlCollectCalendarPeriodEvidence(panel, config, base);
...
async function wlCheckForSession() {
  try {
    ...
    try {
      const maximumMilliseconds = config.target_test_date ? 150000 : 900000;
